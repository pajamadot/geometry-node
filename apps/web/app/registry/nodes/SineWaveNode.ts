...
// SINE WAVE NODE - Generates sine wave animation driver
export const sineWaveNodeDefinition: NodeDefinition = {
  type: 'sine-wave',
  name: 'Sine Wave',
  ...
  inputs: [
    ...
    },
    {
      ...
      type: 'number',
      defaultValue: 1,
      description: 'Frequency of the sine wave (cycles per second)'
    },
    {
      id: 'amplitude',
      name: 'Amplitude',
  ...
  outputs: [
    {
      ...
      name: 'Result',
      type: 'number',
      description: 'Sine wave value between -1 and 1'
    },
    {
      id: 'normalized',
      name: 'Normalized',
      type: 'number',
      description: 'Sine wave value normalized to 0-1 range'
    }
  ],
  parameters: [
    {
  ...
  execute: (inputs, parameters) => {
    ...
    // Calculate sine wave: amplitude * sin(2π * frequency * time + phase)
    const sineValue = amplitude * Math.sin(2 * Math.PI * frequency * currentTime + phase);
    
    // Clamp to [-1, 1] range
    const clampedValue = Math.max(-1, Math.min(1, sineValue));
